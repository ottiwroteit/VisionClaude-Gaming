  ...
  var body: some View {
    ZStack {
      ...
      VStack(spacing: 24) {
        ...
        // Escape hatch: when the SDK gets into a stuck-session state
        // that survives an Xcode kill / crash, this re-registers
        // with Meta AI to clear the orphaned DeviceSession without
        // needing an iPhone restart.
        Button {
          Task { await rayBan.forceReset() }
        } label: {

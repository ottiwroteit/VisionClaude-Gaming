...
final class TennisGame: ObservableObject, Game {
  ...
  /// Last shot event for the SceneKit scene. .serve / .playerReturn /
  /// .pointWon / .pointLost. Bumped via shotEventID so consecutive
  /// same-kind events still fire onChange observers.
  ...
  enum ShotKind { case serve, playerReturn, pointWon, pointLost }
  ...
  func handle(_ event: GestureEvent) {
    ...
    case .rally:
      ...
      let winChance =
        (0.35 + Double(event.magnitude) * 0.5) / activeModifier.difficultyMultiplier
        - pressurePenalty
  ...
  private func awardPoint(toPlayer: Bool) {
    if toPlayer {
      ...
      publishShot(.pointWon)
    } else {
      statusLine = "CPU wins the point"
      publishShot(.pointLost)
    }
    phase = .pointOver
    if score >= 4 {
      playerGames += 1
